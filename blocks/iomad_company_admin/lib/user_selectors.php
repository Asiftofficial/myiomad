...
class current_company_course_user_selector extends company_user_selector_base {
    ...
    public function __construct($name, $options) {
        $this->companyid  = $options['companyid'];
        $this->departmentid = $options['departmentid'];
       //mendapatkan nilai selected role id dari parameter options
        $this->rid = $options['rid'];
        parent::__construct($name, $options);
    }

    //menambahkan parameter rid untuk menampung nilai role id
    public function find_users($search, $all = false, $rid = 0) {
        global $CFG, $DB;
        ...
        list($wherecondition, $params) = $this->search_sql($search, 'u');
        $params['companyid'] = $this->companyid;
        //mengecek kondisi nilai rid, kemudian assign nilai roleid ke params berdasarkan pengecekan kondisi
        if ($rid == 0) {
            $params['roleid'] = $this->rid;
        }else {
            $params['roleid'] = $rid;
        }
        ...
        //menambahkan join table role_assignments pada query sql dan menambahkan where condition pada role_assignments sesuai nilai role id
        $sql = " FROM {user} u
                 ...
                 WHERE $wherecondition AND u.suspended = 0
                 AND cu.companyid = :companyid AND ra.roleid = :roleid
                 $coursesql";
